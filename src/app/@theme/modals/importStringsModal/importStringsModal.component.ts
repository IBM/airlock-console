...
@Component({
    ...
    styleUrls: ['./importStringsModal.scss'],
    templateUrl: './importStringsModal.html',
...
export class ImportStringsModal{
    ...
    platforms = ["Android", "iOS", "JSON"];
    selectedActionTypeIndex:number = -1;
    constructor( private _airLockService:AirlockService,
                 private zone:NgZone,
                 private _stringsSrevice: StringsService,
                 private toastrService: NbToastrService,
                 private modalRef: NbDialogRef<ImportStringsModal>,
    ...
    handleValidationError(dontPopValidationErrors:boolean = false,error:any){
        this.loading = false;
        var serverErrorMessage = error.error || "Request failed, try again later.";
        try{
            var errorMessage:string="";
            if(serverErrorMessage.error != null){
                errorMessage += serverErrorMessage.error;
            }
            if(serverErrorMessage.error != null || !dontPopValidationErrors) {
                this.modalRef?.close();
                this.popover?.hide();
    ...
    _savePasted(overwrite:boolean){
        ...
        this._airLockService.copyStrings(this.seasonId,this._airLockService.getCopiedStrings(),overwrite).then(res => {
            ...
            var successMessage ="";
            if((res as any).addedStrings != undefined && (res as any).addedStrings.length != 0){
                var num = (res as any).addedStringsSize;
                if(num == 1){
            ...
            if((res as any).stringsOverride != undefined && (res as any).stringsOverride.length != 0){
                var num = (res as any).stringsOverrideSize;
                if(num == 1){
            ...
            if((res as any).nonConflictingStrings != undefined && (res as any).nonConflictingStrings.length != 0){
                var num = (res as any).nonConflictingStringsSize;
                if(num == 1){
            ...
            if((res as any).stringsInConflict != undefined && (res as any).stringsInConflict.length != 0){
                var num = (res as any).stringsInConflictSize;
                if(num == 1){
