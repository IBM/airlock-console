...
@Component({
    selector: 'edit-open-answer-modal',
    styleUrls: ['./editOpenAnswerModal.scss'],
    templateUrl: './editOpenAnswerModal.html',
    encapsulation: ViewEncapsulation.None
})

export class EditOpenAnswerModal {
    @ViewChild('paceModalContainerDialog') aceModalContainerDialog: AceModal;
    loading: boolean = false;
    product: Product;
    @Input() answer: OpenAnswer;
    origAnswer: OpenAnswer;
    question: Question;
    poll: Poll;
    ...
    constructor(private _airLockService: AirlockService,
                ...
                @Optional() private modalRef: NbDialogRef<EditOpenAnswerModal>,
    ...
    isDirty(): boolean {
        return this.answer && this.origAnswer && !FeatureUtilsService.isOpenAnswerIdentical(this.answer, this.origAnswer);
    }
                ...
                if (this.question.stage == 'PRODUCTION') {
                    ...
                    this.modalService.open(VerifyActionModal, {
                        ...
                        context: {
                            ...
                            verifyModalDialogType: VerifyDialogType.OPEN_ANSWER_TYPE,
                        }
                    }).onClose.subscribe(confirmed => {
                        if (confirmed) {
                            this._save(dontClose, callback);
    ...
    _save(dontClose = false, callback: (answer: boolean) => any = null) {
        ...
        this.question.openAnswer = this.answer;
    ...
    canMoveOn(callback: (answer: boolean) => any) {
        if (this.loaded && this.isDirty()) {
            let message = 'The open answer has changed and not saved. do you wish to save it now?';
    ...
    open(answer: OpenAnswer, question: Question, poll: Poll, availableBranches: AvailableBranches, strInputSchemaSample: string, strUtilitiesInfo: string, questionCell: QuestionCell = null) {
        if (this.loaded && this.isDirty()) {
            let message = 'The open answer has changed and not saved. do you wish to save it now?';
            this.modalService.open(SaveActionModal, {
                closeOnBackdropClick: false,
    ...
    _open(answer: OpenAnswer, question: Question, poll: Poll, availableBranches: AvailableBranches, strInputSchemaSample: string, strUtilitiesInfo: string, questionCell: QuestionCell = null) {
        ...
        this.answer = OpenAnswer.clone(answer);
        this.origAnswer = OpenAnswer.clone(answer);
    ...
    openOnNewTab() {
        var url = '/#/pages/polls/openanswer/' + this._appState.getCurrentProduct() + '/openAnswer/' + this.question.uniqueId;
        console.log(url);
        window.open(url);
    }
