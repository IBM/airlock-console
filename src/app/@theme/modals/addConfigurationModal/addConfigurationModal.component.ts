...
@Component({
    ...
    styleUrls: ['./addConfigurationModal.scss'],
    templateUrl: './addConfigurationModal.html',
...
export class AddConfigurationModal {
    // @ViewChild('myModal')
    // modal: ModalComponent;
    title: string = "Add Feature";
    type: string = "AddFeature";
    ...
    showNameSpaceInTitle: boolean = true;
    namespace: string = '';
    ...
    constructor(private _airLockService: AirlockService,
                ...
                protected modalRef: NbDialogRef<AddConfigurationModal>) {
        this.loading = true;
    ...
    ngOnInit() {
        // this.clean();
        if (this.type === 'AddFeature') {
            console.log("openAsAddSubFeature");
            var newNamespace = "";
        ...
        } else if (this.type === 'ExistingXM') {
            console.log("openInExistingXM");
            if (this.isConfiguraion) {
                this.title = "Add Configuration To Mutual Exclusion Group";
                for (let item of this.mxGroupToAdd.configurationRules) {
                    if (item.namespace) {
                ...
                if (this.isOrderRule) {
                    ...
                    for (let item of this.mxGroupToAdd.orderingRules) {
                        if (item.namespace) {
                            this.feature.namespace = item.namespace;
                        }
                        this.mxItemNames.push(item.name);
        ...
        } else if (this.type === 'OtherFeatureToMX') {
            if (!this.isOrderRule) {
    ...
    saveNewMXGWithNewFeatureAndOther() {
        ...
        this._airLockService.addFeature(mxGroup, this.seasonId, this.branchId, this.parentId).then(result => {
            ...
            this._airLockService.addFeature(newFeature, this.seasonId, branchId, mxId).then(result => {
                ...
                this._airLockService.getFeature(mxId, branchId).then(result => {
                    ...
                    this._airLockService.getFeature(newFetchedFeature.uniqueId, branchId).then(result => {
                        ...
                        this._airLockService.updateFeature(fetchedMX, branchId).then(result => {
                            this.loading = false;
    ...
    saveInExistingMX() {
        ...
        this._airLockService.addFeature(newFeature, this.seasonId, this.branchId, this.parentId).then(result => {
            console.log("after add new feature");
            var newFetchedFeature: Feature = (result as Feature);
            this._airLockService.getFeature(localmxGroupToAdd.uniqueId, branchId).then(result => {
    ...
    save() {
        if (this.isValid()) {
            ...
            if (this.otherFeatureToCreateMX != null) {
                ...
                this._airLockService.addFeature(this.feature, this.seasonId, this.branchId, this.parentId).then(result => {
                    this.loading = false;
    ...
    openAsAddSubFeatureWithParent(branchId: string, parentFeature: Feature, namespace: string, isConfiguraion: boolean = true, isOrderRule: boolean = false) {
        this.openAsAddSubFeature(branchId, parentFeature.uniqueId, parentFeature.name, namespace, isConfiguraion, isOrderRule);
    }

    openAsAddSubFeature(branchId: string, parentId: string, parentName: string, namespace: string, isConfiguraion: boolean = true, isOrderRule: boolean = false) {
