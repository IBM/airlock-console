...
@Component({
    ...
    styleUrls: ['./variantCell.scss', './dnd.scss'],
    templateUrl: './variantCell.html',
...
export class VariantCell {
    ...
    @Output() onEditVariantInline: EventEmitter<Variant> = new EventEmitter<Variant>();
    nextLevel: number;
    ...
    isPartOfSearch(term: string, variant: Variant): boolean {
        ...
        let branchName = this.getBranchName(this.variant.branchName);
        if (branchName) {
    ...
    updateHighlight(hasHit: boolean) {
        ...
        let branchText = this.getBranchName(this.variant.branchName);
    ...
    openEditDialog(event: Event, isInline: boolean = true) {
        ...
        if (isInline) {
            this.onEditVariantInline.emit(this.variant);
            return;
        }
        ...
        if (this.ruleUtilitieInfo != null && this.ruleInputSchemaSample != null && this.availableBranches != null) {
            ...
            this.modalService.open(EditVariantModal, {
                ...
                context: {
                    variant: Variant.clone(this.variant),
            ...
            this._airlockService.getAvailableBranches(this.experiment.uniqueId).then(result => {
                ...
                this._airlockService.getExperimentUtilitiesInfo(this.experiment.uniqueId, this.experiment.stage, this.experiment.minVersion).then(result1 => {
                    this.ruleUtilitieInfo = result1 as string;
                    console.log('UtilityInfo:');
                    console.log(this.ruleUtilitieInfo);
                    this._airlockService.getExperimentInputSample(this.experiment.uniqueId, this.experiment.stage, this.experiment.minVersion).then(result2 => {
    ...
    _changeStage() {
        ...
        this._airlockService.updateVariant(this.variant).then(variant => {
            ...
            this._airlockService.notifyDataChanged("success-notification", {
                title: "Success",
                message: "Variant stage changed successfully"
            });
        }).catch(error => {
    ...
    isSelected() {
        if (this.selectedId && this.selectedId.length > 0 && this.selectedId == this.variant.uniqueId) {
            ...
            this.onSelected.emit({"event": event, "id": this.variant.uniqueId, "offset": y, "nativeElement": this._selector.nativeElement})
            return true;
        } else {
            return false;
        }
