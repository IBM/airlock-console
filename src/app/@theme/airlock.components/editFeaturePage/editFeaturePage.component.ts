...
@Component({
    selector: 'edit-feature-page',
    providers: [FeatureUtilsService, TransparentSpinner, NbTabsetModule],
    styleUrls: ['./editFeaturePage.scss'],
    templateUrl: './editFeaturePage.html',
    encapsulation: ViewEncapsulation.None
})

export class EditFeaturePage {
    ...
    constructor(private _airLockService: AirlockService,
                ...
                @Optional() private modalRef: NbDialogRef<EditFeaturePage>,
    ...
    initAfterClose() {
        ...
        this.purchasesForSelect = null;
        this.selectedPurchaseTitle = null;
    }
        ...
        if (this.selectedParent == null || this.selectedParent.feature.uniqueId == null) {
            ...
            if ((curFeatureInFlatList.parent == null && this.selectedParent.feature != null && this.selectedParent.feature.uniqueId != this.rootId) || (curFeatureInFlatList.parent != null && curFeatureInFlatList.parent.uniqueId != this.selectedParent.feature.uniqueId)) {
                ...
                this._airLockService.getFeature(featureToUpdate.uniqueId, this.branch.uniqueId).then(result => {
                    ...
                    this._airLockService.updateFeature(featureToUpdate, this.branch.uniqueId).then((updatedFeature) => {
                        this.loading = false;
                        //reload needed since we moved it in the tree
                        if (dontClose) {
                            this.refreshEditScreen(updatedFeature, "outputEventWhiteListUpdate");
                        } else {
    ...
    refreshEditScreen(updated: Feature, message = "") {
        this.originalFeature = updated;
        this.feature = Feature.clone(updated);
        this.loading = false;
    }
    ...
    ngOnInit() {
        ...
        if (this.isConfigurationRule()) {
            ...
            if (this.feature && !this.feature.configurationSchema) {
    ...
    setSelectedPurchaseFromFeature() {
        if (this.feature && this.feature.entitlement && this.feature.entitlement.length > 0) {
            let selected = null;
            if (this.inAppPurchases) {
                ...
                for (var purchase of allPurchases) {
                    if (purchase.uniqueId == this.feature.entitlement) {
                        ...
                        this.selectedPurchaseTitle = this.getPurchaseDisplayName(purchase);
                        return;
                    }
                }
                this.selectedPurchase = null;
                this.selectedPurchaseTitle = "";
            }
        } else {
            this.selectedPurchase = null;
            this.selectedPurchaseTitle = "";
    ...
    open(season: Season,
        ...
        if (this.loaded && this.isDirty()) {
            let message = this._featureUtils.getFeatureDisplayName(this.feature) + ' has changed and not saved. do you wish to save it now?';
            this.modalService.open(SaveActionModal, {
                ...
                if (status === 'cancel') {
                    ...
                    this._open(season, branch, feature, featurePath, strInputSchemaSample, strUtilitiesInfo, featureCell, configurationCell, showConfiguration,
                        sourceFeature, orderCell, inAppPurchases, root, showDeepLink);
                }
            });
        } else {
            this._open(season, branch, feature, featurePath, strInputSchemaSample, strUtilitiesInfo, featureCell, configurationCell, showConfiguration,
    ...
    _open(season: Season,
        ...
        if (showConfiguration && !this.isConfigurationRule()) {
            this.ruleTabActive = false;
            this.hirarchyTabActive = false;
            this.generalTabActive = false;
            this.premiumTabActive = false;
    ...
    getPurchasesForSelect(): any[] {
        ...
        if (this.selectedPurchase) {
            let selected = {id: this.selectedPurchase, name: this.getPurchaseDisplayName(this.selectedPurchase)};
            toRet.push(selected);
        }
    ...
    isDirty(): boolean {
        return this.feature && this.originalFeature && !FeatureUtilsService.isFeatureIdentical(this.feature, this.originalFeature);
    }
    ...
    openOnNewTab() {
        window.open('/#/pages/features/' + this._appState.getCurrentProduct() + '/' + this._appState.getCurrentSeason() + '/' + this._appState.getCurrentBranch() + '/' + this.feature
            .uniqueId);
    }
}
