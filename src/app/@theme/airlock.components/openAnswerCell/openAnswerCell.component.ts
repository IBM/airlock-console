...
@Component({
    selector: 'open-answer-cell',
    styleUrls: ['./openAnswerCell.scss', './dnd.scss'],
    templateUrl: './openAnswerCell.html',
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class OpenAnswerCell {
    ...
    @Output() onEditAnswerInline: EventEmitter<OpenAnswer> = new EventEmitter<OpenAnswer>();
    nextLevel: number;
    ...
    isPartOfSearch(term: string, answer: OpenAnswer): boolean {
        if (!term || term == "") {
            return true;
        }
    ...
    delete() {
        ...
        let message = 'Are you sure you want to delete this open answer?';
    ...
    _delete() {
        ...
        questionToUpdate.openAnswer = null;
        this._airlockService.updatePollQuestion(questionToUpdate).then(resp => {
            ...
            this._airlockService.notifyDataChanged("success-notification", {
                title: "Success",
                message: "The open answer was deleted"
            });
        }).catch(error => {
            let errorMessage = this._airlockService.parseErrorMessage(error, "Failed to delete open answer");
            this._airlockService.notifyDataChanged("error-notification", errorMessage);
