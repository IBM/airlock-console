...
@Component({
    selector: 'import-purchases-modal',
   providers: [FeatureUtilsService, TransparentSpinner, NotificationsService, TabsetConfig],
    styles: [require('./importPurchasesModal.scss')],
    template: require('./importPurchasesModal.html'),
    encapsulation: ViewEncapsulation.None

})

export class ImportPurchasesModal{
    ...
    validate(dontPopValidationErrors:boolean = false) {
        ...
        if(this.selectedParent == null){
            this.create(this._stringsSrevice.getString("import_purchases_select_parent"),"Validation Failed");
            return;
        }
        if(this.isPaste){
        ...
        try{
            ...
            if(errorJson.illegalMinAppVersion != null && errorJson.illegalMinAppVersion.length > 0){
                if(this.minAppVersion == null){
                    errorMessage += this._stringsSrevice.getString("import_purchases_provide_minapp_exceed_max");
                }else{
                    errorMessage += this._stringsSrevice.getString("import_purchases_provide_minapp_exceed_max");
                }
            ...
            if(errorJson.illegalName != null && errorJson.illegalName.length > 0){
                if(this.suffix == null){
                    errorMessage += this._stringsSrevice.getString("import_purchases_provide_suffix");
                }else{
                    errorMessage += this._stringsSrevice.getString("import_purchases_change_suffix");
                }
            ...
            if(isMissingAssets){
                errorMessage += this._stringsSrevice.getString("import_purchases_missing_assets");
    ...
    validateCopy(dontPopValidationErrors:boolean = false){
        this.loading = true;
        var targetBranch = (this.targetBranch.uniqueId != null)?this.targetBranch.uniqueId:'MASTER';
        this._airLockService.validateCopyPurchase(this._airLockService.getCopiedPurchase().uniqueId,this._airLockService.getCopiedPurchaseBranch().uniqueId,this.selectedParent.feature.uniqueId,targetBranch,this.suffix,this.minAppVersion).then(res => {
            this.handleValidationSucceed(res);
        })
            .catch(error => {
    ...
    _savePasted(){
        this.loading = true;
        var targetBranch = (this.targetBranch.uniqueId != null)?this.targetBranch.uniqueId:'MASTER';
        this._airLockService.copyPurchase(this._airLockService.getCopiedPurchase().uniqueId,this._airLockService.getCopiedPurchaseBranch().uniqueId,this.selectedParent.feature.uniqueId,targetBranch,this.suffix,this.minAppVersion).then(() => {
            this.loading = false;
            this._airLockService.setCopiedPurchase(null,null);
            this.onImportFeatures.emit(null);
            this.close();
            this._airLockService.notifyDataChanged("success-notification",{title:"Success",message:"Entitlement pasted successfully"});
        })
    ...
    _saveImported() {
        ...
        this._airLockService.importFeature(this.uploadedFileContent,this.selectedParent.feature.uniqueId,targetBranch,this.suffix,this.minAppVersion).then(() => {
            ...
            this.onImportFeatures.emit(null);
            this.close();
            this._airLockService.notifyDataChanged("success-notification",{title:"Success",message:"Entitlement imported successfully"});
        })
            .catch(error => this.handleError(error));
    }
    ...
    open(isPaste:boolean,parent:InAppPurchase) {
        ...
        this.previewOpen=false;
        if(isPaste){
            this.copiedFeatureTitleMain =  this._featureUtils.getFeatureDisplayName(this._airLockService.getCopiedPurchase());
            this.title = this.getString("paste_purchases_title")  +this.copiedFeatureTitleMain;
            this.heading = this.getString("paste_features_tab_title");
            this.copiedFeatureTitlePrefix = "Paste ";
            this.copiedFeatureTitleSuffix  =" entitlement";
            this.saveText = "Paste"

        }else {
            this.showImportFile = true;
            this.title = this.getString("import_purchases_title");
            this.heading = this.getString("import_features_tab_title");
            this.saveText = "Import"
        }
        this.parentFeatureInFlatList = this._featureUtils.getPossiblePurchaseParentsInFlatList(this.rootFeatuteGroups,this.rootId,false);
    ...
    addFeatureToPath(curPath:Array<InAppPurchase>, curFeature: InAppPurchaseInFlatList){
        if(curFeature == null){
            return;
        }
        ...
        if(curFeature.parent != null) {
            var parentAsList: InAppPurchaseInFlatList = this._featureUtils.getInAppPurchaseInFlatListById(this.parentFeatureInFlatList, curFeature.parent.uniqueId);
    ...
    selectParent(parent: InAppPurchaseInFlatList){
        ...
        if(parent.feature.uniqueId != this.feature.uniqueId){
            if(!this._featureUtils.isContainCyclePurchases(parent.feature,this.selectedParent.feature.entitlements)) {
                this.selectedParent = parent;
            }else{
                alert("Can't set entitlement parent to the feature which is his son");
            }
        }else{
            alert("Can't set entitlement parent to the feature itself");
        }
    ...
    selectNewParent(parent: InAppPurchase) {
        ...
        var parentAsList: InAppPurchaseInFlatList = this._featureUtils.getInAppPurchaseInFlatListById(this.parentFeatureInFlatList,parent.uniqueId);
        this.selectedParent = parentAsList;
        var parents:Array<InAppPurchase> = [];
