...
import {EditStreamModal} from "../editStreamModal/editStreamModal.component";

@Component({
    providers: [FeatureUtilsService],
    selector: 'stream-cell',
    styles: [require('./style.scss'),require('./streamCell.scss'), require('./dnd.scss'), require('./animations.scss')],
    template: require('./streamCell.html'),
...
export class StreamCell {
    ...
    @Input() editStreamModal:EditStreamModal;
    ...
    openEditDialog(event){
        ...
        this._airlockService.getStreamUtilitiesInfo(this.stream.seasonId, this.stream.stage, this.stream.minAppVersion).then(result => {
            this.streamUtilitieInfo = result;
            // this.hideIndicator.emit(this.stream);
            // this.editStreamModal.open(this.stream,null,this.streamUtilitieInfo);
            this._airlockService.getStreamInputSample(this.stream.seasonId, this.stream.stage, this.stream.minAppVersion, this.stream.filter).then(result => {
                this.processorInputSchemaSample = result;
                this._airlockService.getStreamFilterInputSample(this.stream.seasonId).then(result => {
                    this.filterInputSchemaSample = result;
                    this.hideIndicator.emit(this.stream);
    ...
    changeEnableState() {
        ...
        if (this.stream.stage=='PRODUCTION') {
            ...
            this.verifyActionModal.open(message,this.stream, VeryfyDialogType.STREAM_TYPE);
        } else {
            this.modal.confirm()
                .title(message)
                .open()
    ...
    _changeEnableState() {
        ...
        this._airlockService.updateStream(this.stream).then(stream => {
            ...
            this._airlockService.notifyDataChanged("success-notification",{title:"Success",message:"Stream "+action+"d successfully"});
        }).catch(error => {
    ...
    _changeStage() {
        ...
        this._airlockService.updateStream(this.stream).then(stream => {
            this.stream = stream;
            this.onUpdate.emit(this.stream);
            this._airlockService.notifyDataChanged("success-notification",{title:"Success",message:"Stream stage changed successfully"});
        }).catch(error => {
    ...
    isSelected() {
        if (this.selectedId && this.selectedId.length > 0 && this.selectedId==this.stream.uniqueId) {
            ...
            this.onSelected.emit({"event":event, "id":this.stream.uniqueId, "offset":y })
            return true;
        } else {
            return false;
        }
