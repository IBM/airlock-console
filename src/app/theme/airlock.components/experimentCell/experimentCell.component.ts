...
import {EditExperimentModal} from "../editExperimentModal/editExperimentModal.component";
...
import {ReorderVariantsModal} from "../reorderVariantsModal/reorderVariantsModal.component";
import {ShowDashboardModal} from "../showDashboardModal/showDashboardModal.component";
import {DatePipe} from "@angular/common";
import {ToastrService} from "ngx-toastr";
import * as moment from "moment";
import {ExperimentsPage} from "../../../pages/experiments/experiments.component";

@Component({
    providers: [FeatureUtilsService, DatePipe],
    selector: 'experiment-cell',
    styles: [require('./style.scss'),require('./experimentCell.scss'), require('./dnd.scss'), require('./animations.scss')],
    template: require('./experimentCell.html'),
...
export class ExperimentCell {
    ...
    @Input() editExperimentModal:EditExperimentModal;
    ...
    , public modal: Modal, private _stringsSrevice: StringsService, private cd: ChangeDetectorRef,
                private _featureUtils:FeatureUtilsService, private _datePipe: DatePipe, private _toastrService: ToastrService,private _experimentsPage:ExperimentsPage) {
    ...
    changeIndexExperiment() {
        ...
        if (this.experiment.stage=='PRODUCTION') {
            ...
            this.verifyActionModal.open(message,this.experiment, VeryfyDialogType.EXPERIMENT_TYPE);
        } else {
            this.modal.confirm()
                .title(message)
                .open()
    ...
    resetDashboard(){
        this.modal.confirm()
            ...
            .body("<p><font size='3' color='white'>"+this.getString("reset_dashboard_alert_message")+"</font></p>")
            .open()
            .catch(err => console.log("ERROR")) // catch error not related to the result (modal open...)
            .then(dialog => dialog.result) // dialog has more properties,lets just return the promise for a result.
            .then(() => {
                ...
                this._airlockService.resetDashboard(this.experiment.uniqueId).then(result => {
                    this._airlockService.notifyDataChanged("success-notification",{title:"Success",message:"Dashboard reset"});
                    this._experimentsPage.loading = false;
                }).catch(error => {
                    this._experimentsPage.loading = false;
                    let errorMessage = this._airlockService.parseErrorMessage(error, "Failed to reset Dashboard");
                    this._airlockService.notifyDataChanged("error-notification",errorMessage);
    ...
    ngOnInit() {
        ...
        let timer = Observable.timer(60*60*1000,60*60*1000); //refresh every hour
        timer.subscribe(t=>this.refreshIndexingInfo());
    ...
    openEditDialog(event){
        ...
        this._airlockService.getExperimentQuota(this.experiment.uniqueId).then(result => {
            ...
            this._airlockService.getAnalyticsForDisplayExperiment(this.experiment.uniqueId).then(result => {
                this.analyticExperiment = result;
    ...
    openAddVariantDialog(){
        console.log('openAddVariantDialog');
    ...
    _changeStage() {
        ...
        this._airlockService.updateExperiment(this.experiment).then(experiment => {
            ...
            this._experimentsPage.loading = false;
            this.onUpdate.emit(this.experiment);
            this._airlockService.notifyDataChanged("success-notification",{title:"Success",message:"Experiment stage changed successfully"});
        }).catch(error => {
            this._experimentsPage.loading = false;
    ...
    isSelected() {
        if (this.selectedId && this.selectedId.length > 0 && this.selectedId==this.experiment.uniqueId) {
            ...
            this.onSelected.emit({"event":event, "id":this.experiment.uniqueId, "offset":y })
            return true;
        } else {
            return false;
        }
